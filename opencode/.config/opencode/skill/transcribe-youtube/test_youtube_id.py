...
@pytest.mark.parametrize("url,expected_id", TEST_URLS)
def test_youtube_url_formats(url, expected_id):
    extracted_id = return_id(url)
    assert extracted_id == expected_id, (
        f"Failed for URL: {url}. Expected '{expected_id}', got '{extracted_id}'"
    )
    assert len(expected_id) == 11, f"Expected ID {expected_id} should be 11 characters"
