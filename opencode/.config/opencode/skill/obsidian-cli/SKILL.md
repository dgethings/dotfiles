...
version: "1.3.0"
description: >
  Use this skill whenever the user wants Opencode to directly interact with their
  ...
  renaming files, finding orphaned notes or broken links. Without this skill, Opencode
  has no way to access vault data or execute vault operations. Treat any request that
  implies "go into my vault and do X" as a trigger — the user is asking Opencode to act,
  ...
  scripts — anything where the user needs an explanation rather than Opencode performing a
  vault operation.
triggers:
  - "obsidian"
  - "vault"
