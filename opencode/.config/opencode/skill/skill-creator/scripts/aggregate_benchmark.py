...
The script supports this directory layout:
    <benchmark_dir>/
    └── eval-N/
        ├── with_skill/
        │   ├── run-1/grading.json
        ...
        for config_dir in sorted(eval_dir.iterdir()):
            ...
            for run_dir in sorted(config_dir.glob("run-*")):
                ...
                if not grading_file.exists():
                    print(
                        f"Warning: grading.json not found in {run_dir}", file=sys.stderr
                    )
                    continue
                ...
                try:
                    ...
                    print(
                        f"Warning: Invalid JSON in {grading_file}: {e}", file=sys.stderr
                    )
                    continue
...
def generate_markdown(benchmark: dict) -> str:
    ...
    lines.append(
        f"| Pass Rate | {a_pr.get('mean', 0) * 100:.0f}% | {b_pr.get('mean', 0) * 100:.0f}% | {delta.get('pass_rate', '—')} |"
    )

    a_time = a_summary.get("time_seconds", {})
    b_time = b_summary.get("time_seconds", {})
    lines.append(
        f"| Time | {a_time.get('mean', 0):.1f}s | {b_time.get('mean', 0):.1f}s | {delta.get('time_seconds', '—')}s |"
    )
