...
Adapted from the Claude Code version to use `opencode run` instead of `claude -p`.
"""
...
from scripts.run_eval import _find_project_root, run_eval
...
def run_loop(
    ...
    if holdout > 0:
        ...
        if verbose:
            print(
                f"Split: {len(train_set)} train, {len(test_set)} test", file=sys.stderr
            )
    else:
        train_set = eval_set
        test_set = []
    ...
    for iteration in range(1, max_iterations + 1):
        ...
        if verbose:
            print(
                f"Train: {train_passed}/{train_total} ({eval_elapsed:.1f}s)",
                file=sys.stderr,
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Run eval + improve loop for skill descriptions"
    )
    ...
    parser.add_argument(
        "--timeout", type=int, default=60, help="Timeout per query (seconds)"
    )
    parser.add_argument(
    ...
    parser.add_argument("--model", required=True, help="Model (provider/model format)")
    parser.add_argument("--verbose", action="store_true")
    parser.add_argument(
        "--report", default="auto", help="HTML report path (auto/none/path)"
    )
    parser.add_argument(
        "--results-dir", default=None, help="Save outputs to timestamped dir"
    )
    ...
    if args.report != "none":
        if args.report == "auto":
            ...
            live_report_path = (
                ...
                / f"skill_report_{skill_path.name}_{timestamp}.html"
            )
        else:
